...
{
    /// <summary>Screen-space target marker: four corner brackets and a center dot, one instance per target.</summary>
    public sealed class TargetReticle : HudElementBase
    {
        #region Fields

        public const int PoolSize = 16; // max simultaneous target reticles

        const float BoxHalf = 20; // min bracket-box half-size (px); the reticle never shrinks below this

        // screen halves; the box is allowed past the viewport, this only keeps the arithmetic finite
        const float MaxScreenOverflow = 4;
        const float ArmLength = 9; // length of each corner bracket arm (px), clamped to the box half
        const float Thickness = 2; // bracket line thickness (px)
        const float DotSize = 2; // centre dot (square) size (px)

        static readonly Color ReticleColor = new Color(0xBB, 0xE9, 0xF6, 220); // #BBE9F6 (a=220)
        static readonly Color BossColor = new Color(0xED, 0x00, 0xD3, 220); // #ED00D3 (a=220)
